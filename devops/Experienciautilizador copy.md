...
1. **Fase 1: Funil de Entrada (Onboarding do Cidadão):**  
   ...
   * Ao clicar numa pré-inscrição, a aplicação abre um assistente inteligente que  
     **verifica automaticamente se o cidadão já existe** na base de dados (pelo NIF) e se já realizou algum dos cursos nos quais se inscreve.  
       
   * Se o cidadão for novo, o formulário de Inscritos é pré-preenchido, incluindo a informação sobre os cursos nos quais se inscreve. Se já existir, os dados são carregados para atualização (são alterados os campos que contiverem informações diferentes na ficha de pré-inscrição), evitando duplicados. O cidadão é apenas inscrito nos cursos validados pelo técnico.  
...
2. **Fase 2: Gestão Académica:**  
   ...
   * Ele cria uma nova Turma, associa-a a um Curso do catálogo.
   * **Novos Campos Administrativos:** Para garantir a rastreabilidade, o Técnico preenche também os dados oficiais da formação:
...
3. **Fase 3: Matrícula de Formandos:**  
   ...
   * A aplicação apresenta todos os formandos "Ativos" inscritos para esse curso que **ainda não estão nessa turma**, prevenindo erros de matrícula.  
...
5. **Fase 5: Gestão Global de Formandos:**  
   ...
   * O quadro permite selecionar um ou mais cidadãos matriculados. Com base na selecção de cidadãos, esta página permitirá criar: registo de presenças, lista simples de participantes, envio de emails aos participantes, como o email de convocatória, etc.

6. **Fase 6: Gestão Administrativa e Financeira (Dossier e Faturação):**
     
     
   * **Controlo de Dossier Técnico-Pedagógico (Gestão de Evidências):**
     * Para cada turma, existe agora um **Dashboard de Conformidade**. O Técnico verifica rapidamente o estado do dossier.
     * **Automação de Checklist:** Em vez de apenas "marcar cruzes", o Técnico faz o **upload dos ficheiros** (Questionários, Relatórios) diretamente na aplicação. O sistema deteta o ficheiro e marca automaticamente o ponto de controlo correspondente como "Conforme".
     * O sistema alerta visualmente (ícones vermelhos/verdes) para documentos em falta.
     
   * **Processamento de Faturação (Exportação):**
     * O sistema gera uma pré-visualização dos dados de faturação por Turma e Formador.
     * O Técnico valida os dados (Nº Informação, Datas, Valores).
     * O Técnico clica em **"Exportar Ficheiro de Faturação (Excel)"**. Este ficheiro contém os dados formatados para serem enviados para os serviços financeiros/administrativos processarem manualmente a fatura.
     * Após a exportação, o Técnico marca a fatura como "Em Processamento".
...
7. **Fase 7: Integração SIGO (Exportação Manual):**
   ...
   * O sistema realiza validações automáticas (ex: impedir exportação se faltarem classificações).
   * O Técnico clica em **"Exportar Dados SIGO (Excel/CSV)"**. O ficheiro resultante segue a estrutura necessária para ser importado manualmente ou servir de guia para o preenchimento na plataforma externa SIGO.
...
* **Fase de Encerramento e Administração:**  
  ...
  * **Conclusão da Turma:** No seu portal, o Formador pode garantir que todos os sumários e presenças estão preenchidos. Com um clique em "Concluir Turma", ele acede ao link do formulário no qual submete o seu relatório de satisfação e o sistema arquiva a turma, sinalizando ao Técnico que o ciclo está completo.
...
1. A página exibe uma mensagem de processamento: *"A procurar as suas respostas..."*.
    2. Segundos depois, uma notificação visual confirma: **"Sucesso! Respostas validadas."** (Sem necessidade de animações complexas que possam falhar em browsers antigos).
    3. A própria página atualiza-se para exibir as suas respostas, permitindo-lhe revê-las.
    4. Finalmente, os botões de ação são desbloqueados: links para as medalhas digitais da **plataforma de medalhas digitais**. O formando clica e conquista, um a um.  
