...
2. **Parametrização da Comunicação e Logística:**  
   O utilizador configura os Modelos de Comunicação num editor de texto rico (HTML), inserindo placeholders dinâmicos (ex: {Nome\_Formando}) para personalização em massa. Paralelamente, gere o inventário de Tipos de Equipamento e de Tipos de Documento, assegurando que o sistema sabe que documentos exigir no Dossier (ex: Questionários de Avaliação).  
   **⚙️ Implicações no Modelo de Dados:**  
...
1. **Triagem e Admissão (Funil de Entrada):**  
   O Técnico visualiza uma grelha interativa com as Pré-Inscrições pendentes, onde cada linha representa o interesse num Curso e numa Ação (Turma) específica (Data/Local). Ao selecionar um candidato, o sistema valida a unicidade pelo NIF. O Técnico completa a ficha do formando:  
   * Valida as **Habilitações** e a **Situação Profissional**.  
   * Preenche obrigatoriamente a **Profissão** (texto livre), a **Entidade Empregadora** e a **Morada Profissional** completa.  
   Só após este enriquecimento converte o registo em "Inscrito", pronto para matrícula.  
   **⚙️ Implicações no Modelo de Dados:**  
...
2. **Planeamento e Logística de Turmas:**  
   O utilizador cria uma nova Turma num formulário mestre-detalhe. Define os parâmetros temporais (Datas, Horário) e administrativos (Nº Informação, Código da Ação). Numa sub-região do formulário, realiza a Alocação de Equipamentos, selecionando o tipo e a quantidade.  
   **⚙️ Implicações no Modelo de Dados:**  
   * **Tabelas:** LMS_CLASSES, LMS_EQUIPMENTS.  
...
4. **Controlo Administrativo e Financeiro (Dossier e Faturas):**  
   ...
   * **Gestão de Faturação:** Na aba financeira, gere o ciclo de vida das faturas dos formadores. Para cada fatura, regista não só o valor, mas os marcos temporais críticos: **Data de Emissão**, **Data Contabilística** (entrada nas finanças) e a final **Data de Pagamento**. Associa também o **Número da Fatura** para reconciliação externa.  
   **⚙️ Implicações no Modelo de Dados:**  
...
1. **Gestão da Sessão Letiva e Assiduidade:**  
   No final da aula, o Formador acede ao registo da Sessão. Introduz o sumário, cola links para recursos pedagógicos e preenche a grelha de presenças.  
   **⚙️ Implicações no Modelo de Dados:**  
   * **Tabelas:** LMS_SESSIONS, LMS_ATTENDANCE.  
...
3. **Encerramento e o "Link Mágico":**  
   Num momento solene da última sessão, o Formador clica num botão **"Gerar Acesso ao Desafio"**. O sistema cria instantaneamente um **Token Único** para aquela turma. O Formador projeta ou partilha este código com os alunos. É esta chave que permite aos alunos, nos seus dispositivos, entrarem na "Cerimónia de Finalização" e reclamarem as medalhas que o formador acabou de desbloquear para eles. O Formador garante também que o seu CV está atualizado.  
   **⚙️ Implicações no Modelo de Dados:**  
   * **Tabelas:** LMS_CLASSES (`Token_Acesso` ou tabela auxiliar de tokens), LMS_ENTITIES (`URL_CV`).
